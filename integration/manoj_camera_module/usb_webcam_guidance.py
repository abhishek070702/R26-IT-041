...
INTEGRATION_DIR = BASE_DIR.parent
...
# Pi defaults stay small so YOLO does not freeze the preview.
# ==========================================================
...
CAMERA_OPEN_WAIT_SECONDS = float(os.getenv("CAMERA_OPEN_WAIT_SECONDS", "1.0"))
...
def wait_after_camera_open(cap, seconds):
    deadline = time.time() + max(0.0, seconds)
    print(f"Camera open wait: {seconds:.1f}s before capture.")
    ...
    while time.time() < deadline:
        ...
        cv2.imshow("YOLO Page Guidance + Auto Capture", display_frame)
...
def maybe_announce_no_page(voice, no_page_since, last_spoken_guidance, current_time):
    if no_page_since is None:
        return last_spoken_guidance
    if current_time - no_page_since < NO_PAGE_ANNOUNCE_SECONDS:
        return last_spoken_guidance
    if last_spoken_guidance == "No page detected":
        return last_spoken_guidance

    voice.say("No page detected", force=True)
...
def main():
    ...
    if not wait_after_camera_open(cap, CAMERA_OPEN_WAIT_SECONDS):
        cap.release()
    ...
    while True:
        # Drop buffered USB frames so the preview is the latest image.
        cap.grab()
        ret, frame = cap.read()
        ...
        if detection is None:
            ...
            last_spoken_guidance = maybe_announce_no_page(
                voice,
                no_page_since,
                last_spoken_guidance,
                current_time,
            ...
            if is_ready:
                ready_miss_count = 0
                if not session_has_captured and ready_count < READY_STABLE_FRAMES:
                    ready_count = READY_STABLE_FRAMES
                else:
                    ready_count += 1
            else:
                ready_miss_count += 1
            ...
            now = current_time
            if instruction != last_logged_instruction or now - last_log_time > 2.0:
                print(
        ...
        if instruction != "No page detected":
            ...
            # Do not keep repeating "Ready. Hold steady" — that blocks capture speech.
            if (
                stable_text is not None
                and "Ready" not in stable_text
                and stable_text != last_spoken_guidance
            ):
                voice.say(stable_text)
                last_spoken_guidance = stable_text
        ...
        if (
            ...
            voice.say("Capturing page", force=True)
